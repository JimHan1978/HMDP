...
public class GlideImageLoaderStrategy implements BaseImageLoaderStrategy<ImageConfigImpl>, GlideAppliesOptions {
    ...
    public void loadImage(Context context, ImageConfigImpl config) {
        ...
        GlideRequests requests = GlideArch.with(context);
    ...
    public void clear(final Context context, ImageConfigImpl config) {
        ...
        if (config.getImageViews() != null && config.getImageViews().length > 0) {
            ...
            for (ImageView imageView : config.getImageViews()) {
                GlideArch.get(context).getRequestManagerRetriever().get(context).clear(imageView);
            }
        }
            ...
            Observable.just(0)
                    ...
                    .subscribe(new Consumer<Integer>() {
                        ...
                        public void accept(Integer integer) throws Exception {
                            GlideArch.get(context).clearDiskCache();
                        }
                    });
        }
        ...
        if (config.isClearMemory()) {
            ...
            Observable.just(0)
                    ...
                    .subscribe(new Consumer<Integer>() {
                        @Override
                        public void accept(Integer integer) throws Exception {
                            GlideArch.get(context).clearMemory();
                        }
                    });
        }
   }
